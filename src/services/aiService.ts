...
export async function generatePost(input: GeneratePostInput) {
  ...
  instructionParts.push(
    `Target article length: approximately ${maxEstimatedWords} words.`,
  );
      ...
      ? {
          seoTitle:
            typeof (parsed as { seoTitle?: unknown }).seoTitle === "string"
              ...
              : "",
          metaDescription:
            typeof (parsed as { metaDescription?: unknown }).metaDescription ===
            "string"
              ? (parsed as { metaDescription: string }).metaDescription.trim()
              : "",
          focusKeyword:
            typeof (parsed as { focusKeyword?: unknown }).focusKeyword ===
            "string"
          ...
          ogTitle:
            typeof (parsed as { ogTitle?: unknown }).ogTitle === "string"
              ...
              : "",
          ogDescription:
            typeof (parsed as { ogDescription?: unknown }).ogDescription ===
            "string"
...
export async function rewriteSection(input: RewriteSectionInput) {
  ...
  if (input.paragraphIndexInSection !== undefined) {
    const targetParagraph =
      section.paragraphHtmlList[input.paragraphIndexInSection - 1];
  ...
  await prisma.usageLog.create({
    data: {
      ...
      userId: input.userId,
      actionType:
        rewriteMode === "paragraph" ? "rewrite_paragraph" : "rewrite_section",
      modelName,
      inputTokens: response.usage?.input_tokens ?? 0,
      outputTokens: response.usage?.output_tokens ?? 0,
      imageCount: 0,
