...
exports.deleteDraftSync = deleteDraftSync;
...
async function publishDraft(input) {
    ...
    const hasForeignSiteCategories = draft.categories.some((category) => category.siteId !== site.id);
    if (hasForeignSiteCategories &&
        draftCategoryIds.length !== draft.categories.length) {
        throw new Error('Draft categories belong to a different site. Please re-select categories for the target site.');
    }
    const draftTagIds = draft.tags
        .filter((tag) => tag.siteId === site.id)
        .map((tag) => tag.wpTagId);
    const hasForeignSiteTags = draft.tags.some((tag) => tag.siteId !== site.id);
    if (hasForeignSiteTags && draftTagIds.length !== draft.tags.length) {
        throw new Error('Draft tags belong to a different site. Please re-select tags for the target site.');
    }
    ...
    const publishResult = existingSync?.wpPostId
        ? await (0, wordpressService_1.updateWpPost)({
            ...wpAuth,
        ...
        : await (0, wordpressService_1.publishWpPost)({
            ...wpAuth,
            title: draft.title,
            contentHtml: draft.contentHtml,
            excerpt: draft.excerpt,
    ...
    if (!publishResult.success) {
        await prisma_1.prisma.wpPostSync.upsert({
            where: {
                draftId_siteId: {
                    draftId: draft.id,
                    siteId: site.id,
    ...
    const sync = await prisma_1.prisma.wpPostSync.upsert({
        where: {
            draftId_siteId: {
                draftId: draft.id,
                siteId: site.id,
        ...
        update: {
            ...
            syncStatus: targetStatus === 'publish'
                ? client_1.SyncStatus.PUBLISHED
                : client_1.SyncStatus.DRAFT,
...
async function deleteDraftSync(input) {
    const draft = await prisma_1.prisma.draft.findFirst({
        where: {
            id: input.draftId,
            userId: input.userId,
    ...
    const deleteResult = await (0, wordpressService_1.deleteWpPost)({
        siteUrl: site.siteUrl,
        wpUsername: site.wpUsername,
        wpApplicationPassword: (0, crypto_1.decrypt)(site.wpApplicationPasswordEncrypted),
    ...
    if (!deleteResult.success) {
        await prisma_1.prisma.wpPostSync.update({
            where: {
                draftId_siteId: {
                    draftId: draft.id,
                    siteId: site.id,
    ...
    await prisma_1.prisma.wpPostSync.delete({
        where: {
            draftId_siteId: {
                draftId: draft.id,
                siteId: site.id,
